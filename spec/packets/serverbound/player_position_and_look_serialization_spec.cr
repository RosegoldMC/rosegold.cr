...
Spectator.describe "PlayerPositionAndLook Serialization" do
  it "can read and write PlayerPositionAndLook packet with perfect equality" do
    ...
    puts "PlayerPositionAndLook packet serialization test"
    puts "Original packet:"
    puts "  Position: #{original_packet.feet}"
    puts "  Look: yaw=#{original_packet.look.yaw}°, pitch=#{original_packet.look.pitch}°"
    puts "  On ground: #{original_packet.on_ground?}"
    puts "  Pushing against wall: #{original_packet.pushing_against_wall?}"
    
    # Write the packet to bytes
    packet_bytes = original_packet.write
    puts "Packet size: #{packet_bytes.size} bytes"
    puts "Packet bytes: #{packet_bytes.hexstring}"
    ...
    puts "Parsed packet:"
    puts "  Position: #{parsed_packet.feet}"
    puts "  Look: yaw=#{parsed_packet.look.yaw}°, pitch=#{parsed_packet.look.pitch}°"
    puts "  On ground: #{parsed_packet.on_ground?}"
    puts "  Pushing against wall: #{parsed_packet.pushing_against_wall?}"
    ...
    puts "Rewritten bytes: #{rewritten_bytes.hexstring}"
    
    # Compare the bytes for perfect roundtrip
    if packet_bytes == rewritten_bytes
      puts "✅ Perfect match! PlayerPositionAndLook roundtrip successful."
      expect(rewritten_bytes).to eq(packet_bytes)
    else
      puts "❌ Mismatch detected!"
      puts "Expected: #{packet_bytes.hexstring}"
  ...
  it "can parse PlayerPositionAndLook values correctly with protocol 772" do
    ...
    puts "✅ PlayerPositionAndLook values validated successfully"
  end
  ...
  it "handles flag combinations correctly for MC 1.21.8+" do
    ...
    test_cases.each_with_index do |test_case, index|
      ...
      puts "✅ Test case #{index + 1}: on_ground=#{test_case[:on_ground]}, pushing=#{test_case[:pushing]} → flags=0x#{expected_flags.to_s(16).upcase.rjust(2, '0')}"
    end
    
    puts "✅ All flag combinations tested successfully"
  end
  ...
  it "sanitizes invalid coordinate and angle values" do
    ...
    puts "✅ Coordinate and angle sanitization working correctly"
  end
end