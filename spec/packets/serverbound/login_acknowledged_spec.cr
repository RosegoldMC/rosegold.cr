...
Spectator.describe Rosegold::Serverbound::LoginAcknowledged do
  describe "protocol support" do
    it "supports protocol 767 (MC 1.21)" do
      expect(Rosegold::Serverbound::LoginAcknowledged.supports_protocol?(767_u32)).to be_true
      expect(Rosegold::Serverbound::LoginAcknowledged[767_u32]).to eq(0x03_u8)
    end

    it "supports protocol 771 (MC 1.21.6)" do
      expect(Rosegold::Serverbound::LoginAcknowledged.supports_protocol?(771_u32)).to be_true
      expect(Rosegold::Serverbound::LoginAcknowledged[771_u32]).to eq(0x03_u8)
    end
    ...
    it "returns correct supported protocols" do
      ...
      expect(supported).to contain(767_u32)
      expect(supported).to contain(771_u32)
