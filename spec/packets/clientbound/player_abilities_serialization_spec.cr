...
Spectator.describe "PlayerAbilities Serialization" do
  it "can read and write PlayerAbilities packet with perfect equality" do
    ...
    puts "PlayerAbilities packet serialization test"
    puts "Original packet size: #{original_bytes.size} bytes"
    puts "Original bytes: #{original_bytes.hexstring}"
    ...
    puts "Parsed packet:"
    puts "  Flags: 0x#{packet.flags.to_s(16).upcase.rjust(2, '0')}"
    puts "    - Invulnerable: #{packet.invulnerable?}"
    puts "    - Flying: #{packet.flying?}"
    puts "    - Allow Flying: #{packet.allow_flying?}"
    puts "    - Creative Mode: #{packet.creative_mode?}"
    puts "  Flying Speed: #{packet.flying_speed}"
    puts "  Field of View Modifier: #{packet.field_of_view_modifier}"
    ...
    if original_bytes == rewritten_bytes
      puts "✅ Perfect match! PlayerAbilities roundtrip successful."
  ...
  it "can parse PlayerAbilities values correctly" do
    ...
    puts "✅ PlayerAbilities values validated successfully"
  end
  ...
  it "can create PlayerAbilities packets with different flag combinations" do
    ...
    puts "✅ PlayerAbilities flag combinations tested successfully"
  end
end