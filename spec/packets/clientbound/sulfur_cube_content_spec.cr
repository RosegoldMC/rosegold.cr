...
# H1: sulfur_cube_content (protocol 776 / MC 26.2, component id 78).
# Its payload is an ItemStackTemplate (item-then-count + DataComponentPatch),
# the OPPOSITE field order from Slot. Before the fix it had no dispatch arm, so
...
# round-trip, and — critically — that sibling slots survive alongside it.
Spectator.describe Rosegold::DataComponents::SulfurCubeContent do
  ...
  # An absorbed block item carrying its own nested component, to exercise the
  # full ItemStackTemplate DataComponentPatch (not just bare item_id+count).
  let(:absorbed) do
    Rosegold::Slot.new(
      count: 1_u32,
      item_id_int: 42_u32,
  ...
  it "reads as ItemStackTemplate order, not Slot order" do
    # Slot writes count-then-item; ItemStackTemplate writes item-then-count.
    # Reading our template-encoded bytes back through Slot.read (count first)
    # would mis-assign the fields, proving the orders genuinely differ.
    buffer = Minecraft::IO::Memory.new
    ...
    # count was written first as item_id (42); Slot.read treats it as count.
    expect(via_slot.count).to eq(42_u32)
  end
...
Spectator.describe "SetContainerContent with sulfur_cube_content" do
  ...
  it "parses the sulfur slot and preserves sibling slots through a full round-trip" do
    ...
    # Sulfur slot and its embedded ItemStackTemplate survive.
