...
class Rosegold::Serverbound::ChatMessage < Rosegold::Serverbound::Packet
  ...
  property acknowledged : Bytes # BitSet for last seen messages

  def initialize(@message : String, @timestamp : Int64 = Time.utc.to_unix_ms, @salt : Int64 = Random.new.rand(Int64::MIN..Int64::MAX), @signature : Bytes? = nil, @message_count : UInt32 = 0_u32, @acknowledged : Bytes = Bytes.new(0)); end
  ...
  def self.read(io)
    ...
    acknowledged_size = io.read_var_int
    acknowledged = Bytes.new(acknowledged_size)
    io.read_fully(acknowledged) if acknowledged_size > 0
    
    self.new(message, timestamp, salt, signature, message_count, acknowledged)
  end
  ...
  def write : Bytes
    Minecraft::IO::Memory.new.tap do |buffer|
      ...
      buffer.write acknowledged.size
      buffer.write acknowledged if acknowledged.size > 0
    end.to_slice
  end
end
