...
module Rosegold::Packets::ProtocolMapping
  ...
  macro packet_ids(mappings)
    ...
    PROTOCOL_PACKET_IDS = {% if kept_keys.empty? %}({} of UInt32 => UInt32){% else %}{
      {% for i in 0...kept_keys.size %}{{kept_keys[i]}} => {{kept_values[i]}},{% end %}
    }{% end %}
    ...
    def self.packet_id_for_protocol(protocol_version : UInt32) : UInt32
      PROTOCOL_PACKET_IDS[protocol_version]?.try(&.to_u32) ||
        raise "#{self.name} not supported at protocol #{protocol_version} in this build"
    end
