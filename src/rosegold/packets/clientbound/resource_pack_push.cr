...
class Rosegold::Clientbound::ResourcePackPush < Rosegold::Clientbound::Packet
  ...
  property id : UUID, url : String, hash : String, prompt : TextComponent?
  ...
  def callback(client)
    self.class.response_actions(client.resource_pack_response).each do |action|
      client.send_packet! Serverbound::ResourcePackResponse.new(id, action)
    end
  end
  ...
  def self.response_actions(policy : Symbol) : Array(Serverbound::ResourcePackResponse::Action)
    ...
    when :loaded
      [
        Serverbound::ResourcePackResponse::Action::Accepted,
        Serverbound::ResourcePackResponse::Action::SuccessfullyLoaded,
      ]
    else
      Log.warn { "Unknown resource_pack_response policy #{policy.inspect}; declining" }
      [Serverbound::ResourcePackResponse::Action::Declined]
    end
  end
end
