...
class Rosegold::FurnaceMenu < Rosegold::ContainerMenu
  ...
  def quick_move_stack(slot_index : Int32) : Rosegold::Slot
    ...
    if slot_index == 2
      ...
      # Vanilla: try ingredient first, then fuel, then main↔hotbar fallback
      if !move_item_stack_to(slot_index, 0, 1, false)
        if !move_item_stack_to(slot_index, 1, 2, false)
